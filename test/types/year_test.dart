...
import 'package:xsd/src/types/year.dart'; // Adjust import path as necessary
  ...
  group('Year', () {
    group('Constructor and Properties', () {
      ...
      test('should create a valid Year object with timezone', () {
        final y = Year(2023, timezoneOffsetInMinutes: 60);
        expect(y.year, 2023);
      ...
      test('should throw ArgumentError for invalid timezone offset', () {
        expect(
          () => Year(2023, timezoneOffsetInMinutes: -15 * 60),
          throwsArgumentError,
        );
        expect(
          () => Year(2023, timezoneOffsetInMinutes: 15 * 60),
         ...
         expect(
          () => Year(2023, timezoneOffsetInMinutes: -841),
          throwsArgumentError,
        );
        expect(
          () => Year(2023, timezoneOffsetInMinutes: 841),
    ...
    group('Parsing', () {
      test('should parse valid gYear string without timezone', () {
        final y = Year.parse("2023");
        expect(y.year, 2023);
        expect(y.timezoneOffsetInMinutes, isNull);
      });

      test('should parse valid gYear string with Z timezone', () {
        final y = Year.parse("2023Z");
        expect(y.year, 2023);
        expect(y.timezoneOffsetInMinutes, 0);
      });

      test(
        'should parse valid gYear string with positive timezone offset',
        () {
          final y = Year.parse("2023+05:30");
          expect(y.year, 2023);
      ...
      test(
        'should parse valid gYear string with negative timezone offset',
        () {
          final y = Year.parse("2023-08:00");
          expect(y.year, 2023);
      ...
      test(
        'should parse valid gYear string with 14:00 timezone offset',
        () {
          final y = Year.parse("2023+14:00");
          expect(y.year, 2023);
      ...
      test('should parse long negative year > 4 digits with timezone', () {
        final y = Year.parse("-12023+02:30");
        expect(y.year, -12023);
      ...
      test('should throw FormatException for invalid year format', () {
        expect(() => Year.parse("202"), throwsFormatException); // Too short
        expect(() => Year.parse("2023-"), throwsFormatException); // Dangling hyphen
        expect(() => Year.parse("2023+5:30"), throwsFormatException); // Invalid timezone (hour needs 2 digits)
        expect(() => Year.parse("2023+05:60"), throwsFormatException); // Invalid timezone (minute > 59)
        expect(() => Year.parse("2023+15:00"), throwsFormatException); // Invalid timezone (hour > 14)
        expect(() => Year.parse("2023-15:00"), throwsFormatException); // Invalid timezone (hour > 14)
        expect(() => Year.parse("001-01"), throwsFormatException); // Year too short (non-negative)
        expect(() => Year.parse("-001-01"), throwsFormatException); // Year too short (negative)
        expect(() => Year.parse("2023X"), throwsFormatException); // Invalid timezone char
        expect(() => Year.parse("2023+0500"), throwsFormatException); // Missing colon in timezone
        expect(() => Year.parse("Z"), throwsFormatException); // Only timezone
        expect(() => Year.parse("-Z"), throwsFormatException); // Invalid year with Z
        expect(() => Year.parse("2023+12:345"), throwsFormatException); // Too many minute digits
      });
    });

    group('toString()', () {
      test('should return canonical format without timezone', () {
        expect(Year(2023).toString(), "2023");
      });

      test('should return canonical format with Z timezone', () {
        expect(
          Year(2023, timezoneOffsetInMinutes: 0).toString(),
          "2023Z",
        );
      });
      ...
      test('should return canonical format with positive timezone offset', () {
        expect(
          Year(2023, timezoneOffsetInMinutes: 330).toString(), // +05:30
          "2023+05:30",
        );
      });
      ...
      test('should return canonical format for year 0 (1 BCE)', () {
        expect(Year(0).toString(), "0000");
      });
       test('should return canonical format for year 0 with Z timezone', () {
        expect(Year(0, timezoneOffsetInMinutes: 0).toString(), "0000Z");
      });

      test('should return canonical format for negative year -1 (2 BCE)', () {
        expect(Year(-1).toString(), "-0001");
      });
      test('should return canonical format for negative year -12 (13 BCE)', () {
        expect(Year(-12).toString(), "-0012");
      });
       test('should return canonical format for negative year -123 (124 BCE)', () {
        expect(Year(-123).toString(), "-0123");
      });
       test('should return canonical format for negative year -1234 (1235 BCE)', () {
        expect(Year(-1234).toString(), "-1234");
      });

      test('should return canonical format for long positive year > 4 digits', () {
        expect(Year(12023).toString(), "12023");
      });
       test('should return canonical format for long positive year > 4 digits with timezone', () {
        expect(Year(12023, timezoneOffsetInMinutes: 150).toString(), "12023+02:30");
      });

      test('should return canonical format for long negative year > 4 digits', () {
        expect(Year(-12023).toString(), "-12023");
      });
        test('should return canonical format for long negative year > 4 digits with timezone', () {
        expect(Year(-12023, timezoneOffsetInMinutes: -60).toString(), "-12023-01:00");
      });
    });

    group('Equality and hashCode', () {
      test('should be equal for same year and timezone', () {
        final y1 = Year(2023, timezoneOffsetInMinutes: 60);
        final y2 = Year(2023, timezoneOffsetInMinutes: 60);
        expect(y1, equals(y2));
    ...
    group('copyWith', () {
      final original = Year(2023, timezoneOffsetInMinutes: 60);
